...
function CompanyDashboard({ user, onNavigate }) {
  ...
  const handleApplicationStatusUpdate = async (applicationId, status) => {
    // If status is Hired or Rejected, open message modal
    if (status === 'Hired' || status === 'Rejected') {
      setMessageForm({
        applicationId,
        status,
        message: status === 'Hired' 
          ? 'Congratulations! We are pleased to inform you that you have been selected for this position. Please contact us to discuss the next steps.'
  ...
  const handleMessageSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_BASE_URL}/applications/company/${messageForm.applicationId}/status`, {
        method: 'PATCH',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        ...
        body: JSON.stringify({
          ...
          contact_email: messageForm.contact_email
        })
      });

      if (response.ok) {
        setShowMessageModal(false);
        setMessageForm({ applicationId: null, status: '', message: '', contact_email: '' });
        fetchCompanyData();
        alert('Application status updated and message sent successfully!');
      } else {
        const error = await response.json();
        alert(error.message || 'Status update failed');
      }
  ...
  return (
    <Container className="mt-5 pt-4">
      ...
      {/* Message Modal for Hired/Rejected Status */}
      <Modal show={showMessageModal} onHide={() => setShowMessageModal(false)} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>
            <i className={`fas ${messageForm.status === 'Hired' ? 'fa-check-circle text-success' : 'fa-times-circle text-danger'} me-2`}></i>
            {messageForm.status === 'Hired' ? 'Send Hire Notification' : 'Send Rejection Notice'}
          </Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleMessageSubmit}>
          <Modal.Body>
            <Alert variant={messageForm.status === 'Hired' ? 'success' : 'warning'}>
              <Alert.Heading>
                {messageForm.status === 'Hired' ? 'Congratulations Message' : 'Rejection Notice'}
              </Alert.Heading>
              This message will be sent to the student to inform them about their application status.
              {messageForm.status === 'Hired' && ' Please include your contact information for next steps.'}
            </Alert>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control
                ...
                required
                placeholder={
                  messageForm.status === 'Hired' 
                    ? 'Write a congratulatory message and include next steps...'
                    : 'Write a professional rejection message with feedback if possible...'
                }
              />
              <Form.Text className="text-muted">
                Be professional and specific. {messageForm.status === 'Hired' ? 'Include contact details and next steps.' : 'Provide constructive feedback when possible.'}
              </Form.Text>
            </Form.Group>

            {messageForm.status === 'Hired' && (
              <Form.Group className="mb-3">
                <Form.Label>Contact Email for Follow-up</Form.Label>
                <Form.Control
                  type="email"
                  value={messageForm.contact_email}
                  onChange={(e) => setMessageForm({...messageForm, contact_email: e.target.value})}
                  placeholder="hr@company.com"
                />
                <Form.Text className="text-muted">
                  Provide a direct contact email for the student to reach out for next steps.
                </Form.Text>
              </Form.Group>
            )}

            <div className="bg-light p-3 rounded">
              <strong>Status Change:</strong> Application will be marked as <Badge bg={messageForm.status === 'Hired' ? 'success' : 'danger'}>{messageForm.status}</Badge>
            </div>
          </Modal.Body>
          <Modal.Footer>
            ...
            <Button 
              variant={messageForm.status === 'Hired' ? 'success' : 'danger'} 
              type="submit"
            >
              <i className={`fas ${messageForm.status === 'Hired' ? 'fa-paper-plane' : 'fa-times'} me-2`}></i>
              {messageForm.status === 'Hired' ? 'Send Hire Notice' : 'Send Rejection Notice'}
            </Button>
          </Modal.Footer>
        </Form>
      </Modal>
